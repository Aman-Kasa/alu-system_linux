...
void print_child_buffer(pid_t pid, unsigned long addr, unsigned long size)
{
	...
	for (i = 0; i < limit; i++)
	{
		if (i % 8 == 0) word = ptrace(PTRACE_PEEKDATA, pid, addr + i, NULL);
...
void print_stat_struct(pid_t pid, unsigned long addr)
{
	struct stat st;
	unsigned long i;
	for (i = 0; i < sizeof(struct stat); i += 8)
	{
		long word = ptrace(PTRACE_PEEKDATA, pid, addr + i, NULL);
		memcpy((char *)&st + i, &word, 8);
	}
	printf("{st_mode=");
	if (S_ISREG(st.st_mode)) printf("S_IFREG");
	else if (S_ISDIR(st.st_mode)) printf("S_IFDIR");
	else if (S_ISFIFO(st.st_mode)) printf("S_IFIFO");
	printf("|0%o, st_size=%ld, ...}", st.st_mode & 0777, (long)st.st_size);
}
...
int main(int argc, char **argv, char **envp)
{
	...
	while (1)
	{
		...
		if (WIFSTOPPED(status) && WSTOPSIG(status) == (SIGTRAP | 0x80))
		{
			...
			{
				printf("%s(", nm);
				if (strcmp(nm, "fstat") == 0)
				{
					printf("%ld, ", (long)r.rdi);
					print_stat_struct(pid, r.rsi);
				}
				else if (strcmp(nm, "read") == 0 || strcmp(nm, "write") == 0)
				{
					printf("%ld, ", (long)r.rdi);
