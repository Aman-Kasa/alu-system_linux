...
#define NSYSCALLS (sizeof(syscalls_64_g) / sizeof(syscalls_64_g[0]))

/**
 * get_syscall - safely fetches a syscall_t entry by its number
 * @nr: syscall number (orig_rax)
 *
 * Return: pointer to the matching entry, or NULL if out of range
 * (protects against syscalls made by newer kernels/libc that are not
 * present in this table)
 **/
static __attribute__((unused)) const syscall_t *get_syscall(unsigned long nr)
{
	if (nr >= NSYSCALLS)
		return (NULL);
