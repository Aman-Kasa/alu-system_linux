 ...
 *                 interpreting NULL and known flag macros where relevant
 * @sc: the syscall being printed (NULL for unknown syscalls)
 * @name: syscall name to special-case flag interpretation (may differ
 *        from sc->name when formatting a return value)
 * @idx: parameter index, or (size_t)-1 for a return value
 * @type: the type to use for formatting
 * @val: the raw register value
 * @pid: pid of the traced process
 **/
void print_one_arg(const syscall_t *sc, const char *name, size_t idx,
	type_t type, size_t val, pid_t pid)
{
	char *str;
...
int main(int argc, char *argv[], char *envp[])
{
	...
	else
	{
		for (status = 1, skip = 0; !WIFEXITED(status); skip ^= 1)
		{
			...
			if (skip)
			{
				...
				for (i = 0; sc->params[0] != VOID && i < sc->nb_params; i++)
				{
					...
					if (sc->params[i] == VARARGS)
						...
						print_one_arg(sc, sc->name, i, sc->params[i],
							params[i], pid);
				}
			}
			...
			{
				printf(") = ");
				print_one_arg(sc, "", (size_t)-1, sc->ret, regs.rax, pid);
			}
		}
	}
