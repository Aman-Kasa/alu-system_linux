 ...
 */
static const char *arch_str32(uint16_t machine)
{
	if (machine == EM_386)
		return ("i386");
	if (machine == EM_ARM)
...
static const char *fmt_str32(uint16_t machine, unsigned char endian)
{
	if (machine == EM_386)
		return ("elf32-i386");
	if (machine == EM_ARM)
		return (endian == ELFDATA2MSB ? "elf32-bigarm" : "elf32-littlearm");
	if (machine == EM_PPC)
		return ("elf32-powerpc");
	if (machine == EM_SPARC || machine == EM_SPARC32PLUS)
		return ("elf32-sparc");
	if (machine == EM_MIPS)
		return (endian == ELFDATA2MSB ? "elf32-bigmips" : "elf32-littlemips");
...
static void print_flags32(Elf32_Ehdr *ehdr, int has_syms, int sw)
{
	...
	printf("architecture: %s, flags 0x%08x:\n",
	       arch_str32(od_swap16(ehdr->e_machine, sw)), flags);
	if (type == ET_EXEC)
	{
		printf("%sEXEC_P", sep);
		sep = ", ";
...
void hobjdump32(void *ptr, const char *filename)
{
	...
	print_flags32(ehdr, has_syms, sw);
	for (i = 0; i < shnum; i++)
	{
